...
class _ConnectPageState extends State<ConnectPage> {
  ...
  Future<void> _writePrefs() async {
   ...
   getToken(BuildContext ctx) async {
     setState(() {
        _busy = true;
      });
    try {
      ...
      final dio = Dio();
      var result = await dio.post("http://8.213.39.141:5000/generate-token/$username", options: Options(headers: header));
      
      print(result.data["access_token"]);
      print("hello");
      _tokenCtrl.text = result.data["access_token"];
      _connect(ctx);
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  ElevatedButton(
                    onPressed: _busy ? null : () => getToken(context),
                    style: ElevatedButton.styleFrom(
                      backgroundColor: Color(0xFF5A8BFF),
                      padding: const EdgeInsets.symmetric(
                        vertical: 15,
                      ),
                    ),
